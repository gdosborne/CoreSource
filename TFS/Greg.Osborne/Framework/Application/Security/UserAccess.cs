...
namespace GregOsborne.Application.Security
{
    ...
    public static class UserAccess
    {
        public static bool IsUserInAdminGroup()
        {
            ...
            try {
                if (!NativeMethods.OpenProcessToken(Process.GetCurrentProcess().Handle, NativeMethods.TOKEN_QUERY | NativeMethods.TOKEN_DUPLICATE, out hToken))
                ...
                if (hTokenToCheck == null && !NativeMethods.DuplicateToken(hToken, SECURITY_IMPERSONATION_LEVEL.SecurityIdentification, out hTokenToCheck))
                    throw new Win32Exception();
