...
namespace GregOsborne.Application.Media {
    ...
    public static class Extensions {
        ...
        public static ImageBrush GetImageBrush(string bitmapFilename, double opacity = 0.5) {
            if (bitmapFilename == null || !File.Exists(bitmapFilename)) {
                throw new ArgumentNullException(nameof(bitmapFilename));
            }
            var bmpImage = GetBitmapImage(bitmapFilename);
            var result = new ImageBrush(bmpImage) {
                Opacity = opacity,
