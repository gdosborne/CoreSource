namespace GregOsborne.Controls {
    ...
    public class NumericOnlyTextBox : TextBox {
            ...
            if (!IsDecimalAllowed) {
                ...
                if (Text.ToCharArray().Contains('.')) {
                    if (double.TryParse(Text, out var temp)) {
                        var val = Convert.ToInt32(Math.Round(temp, 0));
                        Text = val.ToString();
                    }
                }
            }
        ...
        //        this.EnterHit(int.Parse(this.Text));
        ...
        private void EnterHit(int number) => ValueEntered?.Invoke(this, new ValueEnteredEventArgs(number));
