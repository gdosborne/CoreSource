...
namespace CongregationExtension.ViewModels {
    public class RecycleBinWindowViewModel : LocalBase {
        ...
        private void RecycleTheItem(object state) {
            ...
            if (Common.Dialogs.Helpers.ShowYesNoDialog(null, title, msg, WpfDialogs.TaskDialogIcon.Shield, width)) {
                ...
                if (file.Exists) {
                    ...
                    SelectedItem = null;
                    App.logger.LogMessage(new StringBuilder($"Restored recycle bin object {group.Name}"), Common.Logging.ApplicationLogger.EntryTypes.Information);
                }
            }
        }
        #endregion
        ...
        private void ClearItem(object state) {
            ...
            if (Common.Dialogs.Helpers.ShowYesNoDialog(null, title, msg, WpfDialogs.TaskDialogIcon.Shield)) {
                var file = new FileInfo(SelectedItem.RecycleFileName);
                if (file.Exists) {
                    file.Delete();
                    var dir = file.Directory;
                    ...
                    App.logger.LogMessage(new StringBuilder($"Deleted recycle bin object {group.Name}"), Common.Logging.ApplicationLogger.EntryTypes.Information);
                    SelectedItem = null;
                }
            }
        }
