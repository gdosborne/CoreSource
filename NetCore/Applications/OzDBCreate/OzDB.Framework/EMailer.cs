namespace OzDB.Application {
	...
	using static OzDB.Application.MailTracker;
	using static OzDB.Application.Session;

	public class EMailer : IDisposable {
		public EMailer(string smtpServer, TimeSpan cacheExpire, MatchingFlags flags) {
			this.addresses = new Dictionary<string, MailAddress>();
			this.SMTPServer = smtpServer;
			this.Tracker = new MailTracker(cacheExpire, flags);
		}
		...
		public EMailer AddAddress(string name, string address, string displayName = null) {
			this.addresses.Add(name, new MailAddress(address, displayName));
			return this;
		}
		...
		public string[] GetMailAddressesRaw(params string[] keys) => this.addresses.Where(x => keys.Contains(x.Key)).Select(x => x.Value.Address).ToArray();

		public MailAddress GetMailAddress(string key) => this.addresses.ContainsKey(key) ? this.addresses[key] : null;

		public string GetMailAddressRaw(string key) => this.GetMailAddress(key).Address;
		...
		private MailAddress GetAddress(string emailAddress) {
			if (!this.addresses.Any(x => x.Value.Address.Equals(emailAddress, StringComparison.OrdinalIgnoreCase))) {
				return new MailAddress(emailAddress);
			}

			return this.addresses.First(x => x.Value.Address.Equals(emailAddress, StringComparison.OrdinalIgnoreCase)).Value;
		}

		private MailAddress[] GetAddresses(string emailAddress) {
			emailAddress = emailAddress.Replace(";", ",");
			return this.GetAddresses(emailAddress.Split(','));
		}

		private MailAddress[] GetAddresses(string[] emailAddress) => emailAddress.Select(x => this.GetAddress(x)).ToArray();

		public bool SafeSendEmail(MatchingFlags flags, params KeyValuePair<MatchingFlags, object>[] data) {
			var result = false;
			if (this.Tracker.IsMailOkToSend(flags, data)) {
				...
				this.SendEmail(mailTo, mailFrom, mailBody, mailSubject, hasBCC, mailBcc, mailPriority, mailAttachments, mailType);
				result = true;
				this.Tracker.TrackedData.Add(item);
			}
			return result;
		}
		...
		public EMailer SendEmail(string emailto, string fromEmail, string message, string subject, EmailTypes mailType = EmailTypes.Unspecified) {
			var to = this.GetAddresses(emailto);
			var fr = this.GetAddress(fromEmail);
			this.SendEmail(to, fr, message, subject, false, default, MailPriority.Normal, null, mailType);
			return this;
		}
		...
		public EMailer SendEmail(string emailto, string fromEmail, string message, string subject, bool sendBccEmail, string bccEmail, EmailTypes mailType = EmailTypes.Unspecified) {
			this.SendEmail(emailto, fromEmail, message, subject, sendBccEmail, bccEmail, MailPriority.Normal, null, mailType);
			return this;
		}
		...
		public EMailer SendEmail(string emailto, string fromEmail, string message, string subject, bool sendBccEmail, string bccEmail, MailPriority priority, string[] attachmentPaths, EmailTypes mailType = EmailTypes.Unspecified) {
			...
			this.SendEmail(to, fr, message, subject, sendBccEmail, bc, priority, attachmentPaths, mailType);
			return this;
		}
		...
		public EMailer SendEmail(MailAddress[] emailto, MailAddress fromEmail, string message, string subject, bool sendBccEmail, MailAddress bccEmail, MailPriority priority, string[] attachmentPaths, EmailTypes mailType = EmailTypes.Unspecified) {
			try {
				using (var emailMessage = new MailMessage {
					...
					var isAvailable = OzDB.Application.Net.Utilities.IsServerPortAvailable(this.SMTPServer, 25);
					if (isAvailable) {
						using (var client = new SmtpClient(this.SMTPServer, 25)) {
							client.Send(emailMessage);
						}
					} else {
						throw new ApplicationException($"Port 25 not available on {this.SMTPServer}");
					}
