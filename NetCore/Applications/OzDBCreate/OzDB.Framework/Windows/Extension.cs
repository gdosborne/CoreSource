namespace OzDB.Application.Windows {
	using System;
	...
	using static OzDB.Application.NativeMethods;
	...
	public static class Extension {
		...
		public static void SetWindowBounds(this Window win, double left, double top, double width, double height) {
			win.Left = left;
			win.Top = top;
			win.Width = width;
			win.Height = height;
		...
		public static void SetBounds(this Window win, Point location, Size size) => win.SetWindowBounds(location.X, location.Y, size.Width, size.Height);
		...
		public static void SavePosition(this Window win, Settings settings, string sectionName, string itemPrefix) {
			var fullItemNameLeft = $"{itemPrefix}.Left";
			var fullItemNameTop = $"{itemPrefix}.Top";
			var fullItemNameWidth = $"{itemPrefix}.Width";
			var fullItemNameHeight = $"{itemPrefix}.Height";

			if (!double.IsInfinity(win.RestoreBounds.Left) && !double.IsNegativeInfinity(win.RestoreBounds.Left)) {
				settings.AddOrUpdateSetting(sectionName, fullItemNameLeft, win.RestoreBounds.Left);
			}
			if (!double.IsInfinity(win.RestoreBounds.Top) && !double.IsNegativeInfinity(win.RestoreBounds.Top)) {
				settings.AddOrUpdateSetting(sectionName, fullItemNameTop, win.RestoreBounds.Top);
			}
			if (!double.IsInfinity(win.RestoreBounds.Width) && !double.IsNegativeInfinity(win.RestoreBounds.Width)) {
				settings.AddOrUpdateSetting(sectionName, fullItemNameWidth, win.RestoreBounds.Width);
			}
			if (!double.IsInfinity(win.RestoreBounds.Height) && !double.IsNegativeInfinity(win.RestoreBounds.Height)) {
				settings.AddOrUpdateSetting(sectionName, fullItemNameHeight, win.RestoreBounds.Height);
			}
		}
	}
}
