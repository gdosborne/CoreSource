...
namespace Common.AppFramework.Primitives {
    public static class Extension {
        ...
        public static int IndexOfIgnoreCase(this IEnumerable<string> items, string value) {
            int? result = null;
            if (!items.Any(x => x.Equals(value, StringComparison.OrdinalIgnoreCase))) {
                result = items.ToList().IndexOf(items.First(x => x.Equals(value, StringComparison.OrdinalIgnoreCase)));
            }
            return result.HasValue ? result.Value : -1;
        }

        public static bool IsNumeric(this string theValue) =>
            double.TryParse(theValue, out var _);
        ...
        public static string RandomHexValue(this string prefix, int length = 8) => 
            new Random().Next().ToString("X").PadLeft(length, '0');
    }
}
